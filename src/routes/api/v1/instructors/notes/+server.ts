...
export const POST: RequestHandler = async ({ request, locals }) => {
	...
	if (!courseId) return API_ERRORS.badRequest('courseId is required.');

	const semester = await getActiveSemester();

	const canManage = await canManageCourse(locals.staff, courseId, semester.id);
	...
	try {
		...
		if (err instanceof ContentUploadError) return API_ERRORS.badRequest(err.message);
		throw err;
	}
	...
	const note = await db.note.create({
		data: {
			...
			chapterLabel: typeof chapterLabel === 'string' && chapterLabel.trim() ? chapterLabel.trim() : null,
	...
	return json({ id: note.id }, { status: 201 });
};
